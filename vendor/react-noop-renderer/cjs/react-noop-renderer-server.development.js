...
var ReactNoopServer = ReactFizzServer({
  ...
  writeStartCompletedSuspenseBoundary: function (destination, suspenseInstance) {
    suspenseInstance.state = 'complete';
    var parent = destination.stack[destination.stack.length - 1];
    parent.children.push(suspenseInstance);
    destination.stack.push(suspenseInstance);
  },
  writeStartPendingSuspenseBoundary: function (destination, suspenseInstance) {
    suspenseInstance.state = 'pending';
    var parent = destination.stack[destination.stack.length - 1];
    parent.children.push(suspenseInstance);
    destination.stack.push(suspenseInstance);
  },
  writeStartClientRenderedSuspenseBoundary: function (destination, suspenseInstance) {
    suspenseInstance.state = 'client-render';
    var parent = destination.stack[destination.stack.length - 1];
    parent.children.push(suspenseInstance);
    destination.stack.push(suspenseInstance);
  },
  writeEndSuspenseBoundary: function (destination) {
    destination.stack.pop();
  },
  writeStartSegment: function (destination, responseState, id) {
    var segment = {
      children: []
    };
...
function render(children, options) {
  ...
  var request = ReactNoopServer.createRequest(children, destination, null, options ? options.progressiveChunkSize : undefined, options ? options.onError : undefined, options ? options.onCompleteAll : undefined, options ? options.onReadyToStream : undefined);
