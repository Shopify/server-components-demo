 ...
 * react-server-dom-esbuild.development.js
 *
...
// TODO: The comments in this file have to be cleaned up
...
function preloadModule(moduleData) {
  ...
  var entry = moduleCache.get(moduleId);

  if (entry === undefined) {
    var thenable = __esbuild_module_load__(moduleId);

    var resolve = moduleCache.set.bind(moduleCache, moduleId, null);
    var reject = moduleCache.set.bind(moduleCache, moduleId);
    thenable.then(resolve, reject);
    moduleCache.set(moduleId, thenable);
  }
} // Actually require the module or suspend if it's not yet ready.
// Increase priority if necessary.

function requireModule(moduleData) {
  var entry = moduleCache.get(moduleData.id);
